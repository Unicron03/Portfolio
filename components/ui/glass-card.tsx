...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  ({ className, glowEffect = true, children, ...props }, ref) => {
    return (
      <div className={cn("relative", className?.includes("h-full") || className?.includes("flex-1") ? "h-full flex-1" : "")}>
        {glowEffect && (
          <div className="absolute -inset-1 rounded-2xl bg-linear-to-r via-red-500/30 blur-xl opacity-70" />
        )}
        <div
          ref={ref}
          className={cn(
            "relative rounded-2xl border border-white/20 h-full",
